        ...
        if "Industry ID" in table.columns and "Label" in table.columns:
            ...
            for record in records:
                ...
                industry["subcategories"] = (
                    hierarchy_parts[1:] if len(hierarchy_parts) > 1 else []
                )
...
async def save_to_json(
    data: dict[str, Any], filepath: str = "industry_codes.json"
) -> None:
    """
    ...
    await asyncio.to_thread(
        lambda: json.dump(
            data, open(filepath, "w", encoding="utf-8"), indent=2, ensure_ascii=False
        )
    )
...
async def main_async():
    ...
    print("Saved to industry_codes.json")
